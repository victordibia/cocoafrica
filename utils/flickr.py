...
def init(credentials):
    global flickr_api
    flickr_api = FlickrAPI(credentials.flickr_key, credentials.flickr_secret, format='parsed-json')

# search flickr api for image
def search_photos_text(search_text, num_max_results, page_num, sort_param):
    search_results = flickr_api.photos.search(text=search_text, per_page=num_max_results, page=page_num, extras=extras,sort=sort_param)
